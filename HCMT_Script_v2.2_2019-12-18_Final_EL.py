...
pFeatureSet = neededLayer.query(where="Report_Al is null and site_currently_empty = 'no' and permission ='yes'")
#pFeatureSet = neededLayer.query(where="site_currently_empty = 'no' and permission ='yes'")
...
for feature in existingFeatures:
  Report_Al=feature.attributes["Report_Al"]#calling Report_AI field
  ...
  #if not Report_Al:
   # Report_Al=""
  #if Report_Al=="":#check if report is empty to do the calculation else go to the next record
  ...
  pcode=feature.attributes["notecalp"]#calling  field
  ...
  if sample_tents!="":
    ...
    while (n<= sample_tents):
      ...
      latrine_cleanliness_HH=feature.attributes["latrine_cleanliness_HH" + str(n)]#calling latrine cleanliness field
  ...
  ##################calculating scoring using the averages#####################
    Hands_washed_score=sum(avg_hands_washed_HH)/sample_tents# sum of all avg households hands washed wrt sample tents
    Disease_prevention_score=sum(avg_disease_prevention_HH)/sample_tents
    source_drinking_score=sum(avg_source_drinking_HH)/sample_tents # sum of all avg households source drinking wrt sample tents
    # sum of all avg households clean latrine wrt sample tent
    clean_latrine_score= sum(avg_latrine_cleanliness_HH)/sample_tents
   ...
   # look_latrine_score = sum(avg_latrine_privacy_HH)/sample_tents #sum of all avg households privacy (look) wrt sample tents
    ...
    Site_score= (open_defecation_score + clean_environment_score + solid_waste_score)/3
    Safe_water_score=(0.25*free_res_tank_score) + (0.5*free_res_HH_score) + (0.25*source_drinking_score)
    total_latrine_score=(clean_latrine_score + fun_latrine_score + lock_latrine_score + light_latrine_score )/4#note that we deleted look_latrine_score so we put 4 instead of 5
    ...
    feature.attributes["Report_Al"]=Report_Al
    #prot sani women
    ...
    print ("Site with data error : "+ pcode )
  #print("Next row ")
length = len(FeaturesToUpdate)
prange = 1
if length % 3 == 0:
  prange =3
if length % 5 == 0:
...
prange = 1
while i <= (length):
  newArr = FeaturesToUpdate[i: (i+ prange)]
  update_result = neededLayer.edit_features(updates=newArr) 
  i+= prange
print("Done")
